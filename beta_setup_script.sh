...
remove_old_bashrc() {
    print_status "Step 4: Removing old .bashrc and creating fresh one..."
    
    local bashrc="$HOME/.bashrc"
    
    # Create backup and remove old .bashrc if it exists
    if [[ -f "$bashrc" ]]; then
        ...
        print_status "Force deleting old .bashrc file..."
