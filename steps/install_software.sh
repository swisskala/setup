...
install_essential_software() {
   ...
   if [[ -f "config/${LLM_REMOTE_FILE}" ]]; then
       ...
       if sudo cp "config/${LLM_REMOTE_FILE}" /usr/bin/llm-remote 2>/dev/null && \
          sudo chmod +x /usr/bin/llm-remote; then
           print_success "llm-remote installed successfully to /usr/bin"
       else
           print_warning "Failed to install llm-remote to /usr/bin"
           print_warning "You can install it manually with:"
           print_warning "  sudo cp config/${LLM_REMOTE_FILE} /usr/bin/llm-remote"
           print_warning "  sudo chmod +x /usr/bin/llm-remote"
       fi
